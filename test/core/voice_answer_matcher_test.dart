...
void main() {
  ...
  group('parseVietnameseNumber', () {
    group('direct digit', () {
      ...
      test(
        'digit "10" → 10 when max=10',
        () => expect(parseVietnameseNumber('10', 10), 10),
      );
      test(
        'digit out of range → null',
        () => expect(parseVietnameseNumber('7', 5), null),
      );
      test(
        'digit "0" → null (VI range starts at 1)',
        () => expect(parseVietnameseNumber('0', 5), null),
      );
    });
    ...
    group('Vietnamese number words with diacritics', () {
      ...
      test(
        'sáu → 6 (max=10)',
        () => expect(parseVietnameseNumber('sáu', 10), 6),
      );
      test(
        'bảy → 7 (max=10)',
        () => expect(parseVietnameseNumber('bảy', 10), 7),
      );
      test(
        'tám → 8 (max=10)',
        () => expect(parseVietnameseNumber('tám', 10), 8),
      );
      test(
        'chín → 9 (max=10)',
        () => expect(parseVietnameseNumber('chín', 10), 9),
      );
      test(
        'mười → 10 (max=10)',
        () => expect(parseVietnameseNumber('mười', 10), 10),
      );
    });
    ...
    group('no-diacritics STT output', () {
      ...
      test(
        'sau → 6 (max=10)',
        () => expect(parseVietnameseNumber('sau', 10), 6),
      );
      test(
        'bay → 7 (max=10)',
        () => expect(parseVietnameseNumber('bay', 10), 7),
      );
      test(
        'tam → 8 (max=10)',
        () => expect(parseVietnameseNumber('tam', 10), 8),
      );
      test(
        'chin → 9 (max=10)',
        () => expect(parseVietnameseNumber('chin', 10), 9),
      );
      test(
        'muoi → 10 (max=10)',
        () => expect(parseVietnameseNumber('muoi', 10), 10),
      );
    });
    ...
    group('mixed text', () {
      test(
        'câu số ba → 3',
        () => expect(parseVietnameseNumber('câu số ba', 5), 3),
      );
      test(
        'tôi chọn năm → 5',
        () => expect(parseVietnameseNumber('tôi chọn năm', 5), 5),
      );
    });

    group('fuzzy diacritics removal', () {
      test(
        'muoi (no diacritics of mười) → 10',
        () => expect(parseVietnameseNumber('muoi', 10), 10),
      );
    });
    ...
    group('maxValue capping', () {
      test(
        'năm=5 capped at max=3 → null',
        () => expect(parseVietnameseNumber('năm', 3), null),
      );
      test(
        'mười=10 capped at max=5 → null',
        () => expect(parseVietnameseNumber('mười', 5), null),
      );
    });
    ...
    group('garbage input', () {
      ...
      test(
        'random text → null',
        () => expect(parseVietnameseNumber('xin chào thế giới', 5), null),
      );
      test(
        'English word → null',
        () => expect(parseVietnameseNumber('hello', 5), null),
      );
    });
  });
  ...
  group('parseEnglishNumber', () {
    group('direct digit', () {
      ...
      test(
        '"10" → 10 (max=10)',
        () => expect(parseEnglishNumber('10', 10), 10),
      );
      test(
        '"6" out of range → null',
        () => expect(parseEnglishNumber('6', 5), null),
      );
    });
    ...
    group('CLEAR_NUMBERS — canonical words', () {
      ...
      test(
        'seven → 7 (max=10)',
        () => expect(parseEnglishNumber('seven', 10), 7),
      );
      test(
        'eight → 8 (max=10)',
        () => expect(parseEnglishNumber('eight', 10), 8),
      );
      test(
        'nine → 9 (max=10)',
        () => expect(parseEnglishNumber('nine', 10), 9),
      );
      test(
        'ten → 10 (max=10)',
        () => expect(parseEnglishNumber('ten', 10), 10),
      );
    });
    ...
    group('CLEAR_NUMBERS — phrase context', () {
      test(
        ...
        () => expect(parseEnglishNumber('the answer is four', 5), 4),
      );
      test(
        '"I think three" → 3',
        () => expect(parseEnglishNumber('I think three', 5), 3),
      );
    });
    ...
    group('STRICT_TOKENS — whole-transcript homophones', () {
      ...
      test(
        '"Bye!" → 5 (trailing punct stripped)',
        () => expect(parseEnglishNumber('Bye!', 5), 5),
      );
      ...
      test(
        '"thank you" → null (multi-word hallucination)',
        () => expect(parseEnglishNumber('thank you', 5), null),
      );
    });

    group('hallucination blocklist', () {
      test(
        '"thank you for watching" → null',
        () => expect(parseEnglishNumber('thank you for watching', 5), null),
      );
      test(
        '"[music]" → null (marker char)',
        () => expect(parseEnglishNumber('[music]', 5), null),
      );
      test(
        '"(crowd cheering)" → null (marker char)',
        () => expect(parseEnglishNumber('(crowd cheering)', 5), null),
      );
    ...
    group('fuzzy Levenshtein fallback', () {
      test(
        '"nein" → 9 (d=1 from "nine")',
        () => expect(parseEnglishNumber('nein', 10), 9),
      );
      test(
        '"treee" → 3 (d=1 from "three")',
        () => expect(parseEnglishNumber('treee', 5), 3),
      );
    ...
    group('maxValue capping', () {
      test(
        'five=5 capped at max=3 → null',
        () => expect(parseEnglishNumber('five', 3), null),
      );
      test(
        'ten=10 capped at max=5 → null',
        () => expect(parseEnglishNumber('ten', 5), null),
      );
    });
    ...
    group('garbage input', () {
      ...
      test(
        'random gibberish → null',
        () => expect(parseEnglishNumber('asdfqwerty', 5), null),
      );
    });
  });
