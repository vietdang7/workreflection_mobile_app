...
/// Marks a cc_video_jobs row's narration_script as generated by THIS mobile
/// builder. Only jobs bearing this marker are reused from cache (see
/// [isReusableMobileJob]); web-app (Gemini) jobs are ignored so the cached
/// audio never mismatches the deterministic on-screen mobile scenes.
const String kMobileNarrationSource = 'mobile_v1';

/// True if a cc_video_jobs row is a completed, reusable job generated by THIS
/// mobile builder (so its cached audio+SRT matches deterministic mobile scenes).
bool isReusableMobileJob(Map<String, dynamic> row) {
  if (row['status'] != 'completed') return false;
  if (row['audio_url'] == null) return false;
  final script = row['narration_script'];
  return script is Map && script['source'] == kMobileNarrationSource;
}
...
class SupabaseVideoReportRepository implements VideoReportRepository {
  ...
  Future<RawVideoJob?> findCompletedJob(String reportId) async {
    final row = await _client
        ...
        .eq('status', 'completed')
        // Only reuse jobs generated by THIS mobile builder — web-app (Gemini)
        // jobs are invisible here, so the provider generates a fresh mobile job.
        .eq('narration_script->>source', kMobileNarrationSource)
  ...
  Future<RawVideoJob> createAndWait({
    ...
    for (var attempt = 0; attempt < _maxPollAttempts; attempt++) {
      ...
      if (rawPoll is! Map) continue;
      final data = Map<String, dynamic>.from(rawPoll);
