...
final workshopDetailProvider = FutureProvider.autoDispose
    .family<WorkshopDetail?, String>((ref, id) {
      final repo = ref.watch(workshopRepositoryProvider);
      return repo.getWorkshop(id);
    });

/// Current user's registration for a workshop.
final myRegistrationProvider = FutureProvider.autoDispose
    .family<WorkshopRegistration?, String>((ref, workshopId) {
      final repo = ref.watch(workshopRepositoryProvider);
      return repo.getMyRegistration(workshopId);
    });

/// Attachments for a workshop.
final workshopAttachmentsProvider = FutureProvider.autoDispose
    .family<List<WorkshopAttachment>, String>((ref, workshopId) {
      final repo = ref.watch(workshopRepositoryProvider);
      return repo.getAttachments(workshopId);
    });

/// Whether the current user has already submitted a survey for a workshop.
final hasSubmittedSurveyProvider = FutureProvider.autoDispose
    .family<bool, String>((ref, workshopId) {
      final repo = ref.watch(workshopRepositoryProvider);
...
final workshopSurveySetProvider = FutureProvider.autoDispose
    .family<WorkshopSurveySet?, String>((ref, workshopId) {
      final repo = ref.watch(workshopRepositoryProvider);
...
final workshopSurveyResultsProvider = FutureProvider.autoDispose
    .family<WorkshopSurveyResults?, String>((ref, workshopId) async {
      final repo = ref.watch(workshopRepositoryProvider);
...
final myWorkshopsProvider =
    FutureProvider.autoDispose<List<(WorkshopRegistration, WorkshopDetail?)>>((
      ref,
    ) async {
      final repo = ref.watch(workshopRepositoryProvider);
