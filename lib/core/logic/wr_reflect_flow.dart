...
String get kNoticePrompt => tr(
  'Điều gì đang mô tả đúng nhất trạng thái công việc của bạn?',
  'What best describes where you are at work right now?',
);
...
String get kOtherSituationLabel =>
    tr('Điều khác, để tôi tự mô tả', 'Something else, let me describe it');
...
String noticeSubtitle(String? moodLabel) {
  if (moodLabel == null || moodLabel.isEmpty) {
    return tr(
      'Chọn điều gần đúng nhất, không cần hoàn hảo.',
      'Pick the closest one. It does not have to be perfect.',
    );
  }
  return tr(
    'Những tình huống dưới đây thường xuất hiện khi ai đó chọn '
        '"$moodLabel".',
    'These situations often come up for people who pick '
        '"$moodLabel".',
  );
}
...
String get kDetailPrompt => tr(
  'Viết ra bất cứ điều gì vừa xuất hiện trong đầu bạn lúc này.',
  'Write whatever just came to mind.',
);
...
String get kFamiliarStoryIntro => tr(
  ...
  'Plenty of working people have been through stretches that feel like '
...
/// từ đâu.
String get kCustomDetailNote => tr(
  'Không cần đầy đủ, chỉ cần bắt đầu từ: ai, khi nào, và chuyện gì đã xảy ra.',
  'It does not have to be complete. Just start with who, when, and what happened.',
);
...
/// người viết cần điền.
String get kStoryDetailHint => tr(
  'Ví dụ: Sáng nay, trong cuộc họp với..., lúc...',
  'For example: This morning, in the meeting with..., when...',
);

/// Gợi ý trong ô chữ ở nhánh "Điều khác" — dài hơn vì không có câu chuyện nào
/// đứng trước làm mẫu.
String get kCustomDetailHint => tr(
  'Ví dụ: Sáng nay, trong cuộc họp với anh Nam, lúc mình vừa nêu ý kiến thì...',
  'For example: This morning, in the meeting with Nam, right after I gave my view...',
);
...
/// Nhãn bước ở Lớp 2.
String get kInsightAhaEyebrow =>
    tr('Một góc nhìn khác', 'Another way to see it');

/// Vế đầu của câu mở dở. Người dùng viết tiếp phần sau chữ "vì".
String get kInsightStemPrefix =>
    tr('Với tôi, điều này xảy ra vì', 'To me, this happens because');
...
List<String> get kInsightStemSuggestions => [
  ...
  tr(
    ...
    'I am not clear what I am hoping for',
  ),
  tr(
    'mình sợ nói ra sẽ làm mọi thứ căng hơn',
    'I am afraid saying it will make things tenser',
  ),
];

/// Nhãn của lối thoát ở Lớp 1.
String get kInsightSkipLabel => tr(
  'Tạm thời bỏ qua, bạn muốn suy nghĩ thêm',
  'Skip for now, you want to think it over',
);

/// Nhãn nút chính ở Lớp 1.
String get kInsightRevealLabel =>
    tr('Xem một góc nhìn khác', 'See another way to look at it');

/// Nhãn khối "điều bạn vừa viết" ở Lớp 2.
String get kInsightYourWordsLabel =>
...
/// vậy.
String get kInsightNormalizingLabel =>
    tr('Đúc kết phổ biến', 'A common thread');
...
String get kInsightDisagreeAck => tr(
  'Cảm ơn bạn đã cho biết. Góc nhìn này sẽ không được lưu lại. Bạn vẫn có thể '
      'tiếp tục với bước sau nhé.',
  'Thanks for telling us. This view will not be saved. You can still carry '
      'on to the next step.',
);

/// Câu chốt dưới khối aha ở Lớp 2.
String get kInsightAhaNote => tr(
  'Một vấn đề luôn có thể được giải nghĩa theo nhiều cách. Đây là một lăng '
      ...
      'tại.',
  'A problem can always be read more than one way. This is one more lens, '
